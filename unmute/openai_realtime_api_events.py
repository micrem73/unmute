...
# === EVENTI CUSTOM BAMBOLA PARLANTE ===

class UnmuteBambolaCordinoTirato(
    BaseEvent[Literal["unmute.bambola.cordino_tirato"]]
):
    """Evento: cordino della bambola è stato tirato - inizia registrazione"""
    timestamp: int


class UnmuteBambolaCordinoRilasciato(
    BaseEvent[Literal["unmute.bambola.cordino_rilasciato"]]
):
    """Evento: cordino rilasciato - termina registrazione e elabora"""
    timestamp: int


class UnmuteBambolaBufferReady(
    BaseEvent[Literal["unmute.bambola.buffer_ready"]]
):
    """Evento server: buffer audio risposta è pronto"""
    buffer_size: int
    transcription: str
    llm_response: str


class UnmuteBambolaPlaybackStarted(
    BaseEvent[Literal["unmute.bambola.playback_started"]]
):
    """Evento server: inizio riproduzione buffer"""
    pass


class UnmuteBambolaPlaybackCompleted(
    BaseEvent[Literal["unmute.bambola.playback_completed"]]
):
    """Evento server: riproduzione completata"""
    pass
...
ServerEvent = Union[
    ...
    UnmuteBambolaBufferReady,              # <-- AGGIUNGI
    UnmuteBambolaPlaybackStarted,          # <-- AGGIUNGI
    UnmuteBambolaPlaybackCompleted,        # <-- AGGIUNGI
]
...
ClientEvent = Union[
    ...
    UnmuteBambolaCordinoTirato,            # <-- AGGIUNGI
    UnmuteBambolaCordinoRilasciato,        # <-- AGGIUNGI
]
